...
describe('IntegrationService credentials', () => {
  ...
  // A database that refuses the query must degrade to the API, not throw: this
  // runs inside a directive, and an exception here would take the flow with it.
  it('a failing query resolves null instead of throwing, and the API answers', async () => {
    integrationService.isConnected = () => true;
    integrationService.model = {
      findOne: () => { throw new Error('connection lost'); }
    };
    const captured = {};
    integrationService.httpUtils = httpStub(masked, captured);

    assert.strictEqual(await integrationService.getIntegrationFromDb('project-1', 'openai'), null);

    const key = await integrationService.getKeyFromIntegrations('project-1', 'openai', 'a-token');
    assert.strictEqual(key, 'sk-********alue', 'the API is the fallback when the database cannot answer');
  });
