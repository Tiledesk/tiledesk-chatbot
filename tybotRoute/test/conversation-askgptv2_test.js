...
describe('Conversation for AskGPTV2 test', async () => {
  ...
  it('/gpt_success_json_sources (key from integrations) - invokes the askgpt mockup and test the returning attributes', (done) => {
    let listener;
    let endpointServer = express();
    endpointServer.use(bodyParser.json());
    endpointServer.post('/:projectId/requests/:requestId/messages', function (req, res) {
      ...
      util.getChatbotParameters(REQUEST_ID, (err, attributes) => {
        if (err) {
          ...
          assert(typeof attributes["kb_json_sources"] === "object");
          listener.close(() => {
            done();
          });
        }
      ...
      if (!req.body.question) {
        ...
        reply = {
          ...
          citations: [
            {
                "source_id": 0,
                "source_name": "https://guide.tiledesk.com/ai-chatbots-and-automation/knowledge-base/hybrid-search",
                "source_file_name": "Hybrid search | Tiledesk Help Center"
            },
            {
                "source_id": 1,
                "source_name": "https://guide.tiledesk.com/ai-chatbots-and-automation/knowledge-base/hybrid-search",
    ...
    listener = endpointServer.listen(10002, '0.0.0.0', () => {
      ...
      let request = {
        "payload": {
          ...
          "sender": "A-SENDER",
          "recipient": REQUEST_ID,
          "text": '/kb_success_json_sources{"last_user_message":"come ti chiami"}',
          "id_project": PROJECT_ID,
          "metadata": "",
          "request": {
            "request_id": REQUEST_ID
