...
const bots_data = require('./conversation-n8n_bot.js').bots_data;
...
describe('Conversation for n8n test', async () => {
...
//TEST SUCCESS
  it('/n8n success', (done) => {
    ...
    endpointServer.post('/:projectId/requests/:requestId/messages', function (req, res) {
      ...
      console.log('/n8n success message: ', JSON.stringify(message, null,2));
      const command1 = message.attributes.commands[1];
      assert(command1.type === "message");
      assert(command1.message.text === 'n8n status is: 201');
      //-------------------------------------------------------
      ...
      assert(command3.message.text === 'n8n error is: null');
      ...
      getChatbotParameters(REQUEST_ID, (err, attributes) => {
        if (err) {
          ...
          assert(attributes);
          assert(attributes["n8n_status"] === 201);
          assert(attributes["n8n_error"] === null);
          listener.close(() => {
            done();
          });
        }
